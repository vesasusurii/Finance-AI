...
@pytest.mark.asyncio
async def test_get_status_allows_other_users_upload(document_service: DocumentService):
    ...
    assert result.invoice_id == 273


@pytest.mark.asyncio
async def test_get_status_allows_foreign_upload_without_invoice(
    document_service: DocumentService,
):
    upload_row = MagicMock()
    upload_row.id = 141
    upload_row.uploaded_by = 1
    upload_row.original_filename = "statement.pdf"
    upload_row.processing_status = "processing"
    upload_row.mime_type = "application/pdf"
    upload_row.file_size = 1000
